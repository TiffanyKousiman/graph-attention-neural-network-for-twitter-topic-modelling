...
class Data():
    ...
    def load_data(self):
        ...
        # read input matrices
        doc = self.doc_preprocessing(np.loadtxt(input_path + 'content_tfidf_2018_2022.txt')) # document-word tfidf weight matrix X
        # retrieve doc similarity matrix
        if self.sim_metric == 'cosine_tfidf':
            self.dis = np.loadtxt(input_path + 'doc_doc_sim_tfidf.txt')
        elif self.sim_metric == 'cosine_count':
            self.dis = np.loadtxt(input_path + 'doc_doc_sim_count_2018_2022.txt')
        else:
            self.dis = np.loadtxt(input_path + 'doc_doc_sim_jaccard.txt')
        # create adjacency matrix based on cut-off threshold
        input_adj_matrix = np.where(self.dis > self.adj_cut_off, 1, 0)
        ...
        with open(input_path + 'word_word_2018_2022.ppmi', 'rb') as f:
        ...
        voc = np.genfromtxt(input_path + 'vocab_2018_2022.txt', dtype=str)
        # self.dis = np.loadtxt(input_path + 'doc_doc_sim_tfidf.txt')
